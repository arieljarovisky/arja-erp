// Script para verificar y actualizar una suscripción por mp_preapproval_id
// Uso: node scripts/check-subscription-by-mp-id.js <mp_preapproval_id>
...
async function main() {
  const mpPreapprovalId = process.argv[2];

  if (!mpPreapprovalId) {
    console.error("❌ Debes proporcionar el mp_preapproval_id");
    console.log("Uso: node scripts/check-subscription-by-mp-id.js <mp_preapproval_id>");
    process.exit(1);
  }

  console.log(`🔍 Buscando suscripción con mp_preapproval_id: ${mpPreapprovalId}...\n`);
  ...
  if (!subscription) {
    console.error(`❌ No se encontró la suscripción con mp_preapproval_id ${mpPreapprovalId}`);
    process.exit(1);
  }
  ...
  const mpData = await fetchPreapproval(mpPreapprovalId, token);
