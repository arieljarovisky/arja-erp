...
/**
 * GET /api/dashboard/instructors?from=YYYY-MM-DD&to=YYYY-MM-DD
 * KPIs por estilista en rango: cantidad, monto lista, primera y última hora
 */
adminDashboard.get("/dashboard/instructors", async (req, res) => {
  try {
    const tenantId = req.tenant.id;
    ...
    console.error("[GET /dashboard/instructors] error:", e);
    res.status(500).json({ ok:false, error:e.message });
  }
});
