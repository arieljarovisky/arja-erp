...
/**
 * GET /api/reports/instructors?from=YYYY-MM-DD&to=YYYY-MM-DD
 * KPIs por estilista (cantidad, monto, comisión estimada).
 */
reports.get("/reports/instructors", async (req, res) => {
  try {
    const tenantId = req.tenant.id;
    const from = String(req.query.from || "").slice(0, 10);
    const to   = String(req.query.to   || "").slice(0, 10);
    ...
    const [rows] = await pool.query(
      ...
      JOIN service  s ON s.id  = a.service_id  AND s.tenant_id  = a.tenant_id
      LEFT JOIN instructor_commission c 
        ON c.instructor_id = st.id AND c.tenant_id = a.tenant_id
      WHERE a.tenant_id = ?
        AND a.status IN (${placeholders})
        AND a.starts_at BETWEEN ? AND ?
      GROUP BY st.id
    ...
    console.error("[GET /reports/instructors] error:", e);
    res.status(500).json({ ok:false, error: e.message });
  }
});
