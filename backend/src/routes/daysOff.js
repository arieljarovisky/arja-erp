// src/routes/daysOff.js — MULTI-TENANT
import { Router } from "express";
import { pool } from "../db.js";
import { requireAuth, requireRole } from "../auth/middlewares.js";

export const daysOff = Router();

// Seguridad: solo admin y staff
daysOff.use(requireAuth, requireRole("admin", "user"));

/**
 * GET /api/days-off?instructorId=&from=YYYY-MM-DD&to=YYYY-MM-DD
 */
daysOff.get("/", async (req, res) => {
  try {
    const tenantId = req.tenant.id;
    const instructorId = Number(req.query.instructorId);
    const from = req.query.from; // YYYY-MM-DD
    const to = req.query.to;     // YYYY-MM-DD

    if (!instructorId) {
      return res.status(400).json({ ok: false, error: "Falta el parámetro instructorId" });
    }

    // validar estilista del tenant
    const [[sty]] = await pool.query(
      "SELECT id FROM instructor WHERE id=? AND tenant_id=? LIMIT 1",
      [instructorId, tenantId]
    );
    if (!sty) return res.status(404).json({ ok:false, error:"Peluquero no encontrado en tu cuenta" });

    let sql = `
      SELECT id, instructor_id, starts_at, ends_at, reason, created_at
      FROM time_off
      WHERE tenant_id = ? AND instructor_id = ?
    `;
    const params = [tenantId, instructorId];

    if (from) { sql += " AND DATE(starts_at) >= ?"; params.push(from); }
    if (to)   { sql += " AND DATE(starts_at) <= ?"; params.push(to);   }

    sql += " ORDER BY starts_at ASC";

    const [rows] = await pool.query(sql, params);
    return res.json({ ok: true, data: rows });
  } catch (e) {
    console.error("[GET /api/days-off] error:", e);
    return res.status(500).json({ ok: false, error: e.message || "Error al obtener días off" });
  }
});

/**
 * POST /api/days-off/recurring
 * Body: { instructorId, startTime: "HH:MM", endTime: "HH:MM", from: "YYYY-MM-DD", to: "YYYY-MM-DD", reason, weekdays?: [0-6] }
 * Crea bloqueos repetitivos entre un rango de fechas. Si no se especifica weekdays, aplica todos los días.
 */
daysOff.post("/recurring", async (req, res) => {
  try {
    const tenantId = req.tenant.id;
    const body = req.body || {};
    const instructorId = Number(body.instructorId);
    const startTime = String(body.startTime || "");
    const endTime = String(body.endTime || "");
    let from = String(body.from || "");
    let to = String(body.to || "");
    const reason = String(body.reason || "Bloqueo de tiempo");
    const weekdays = Array.isArray(body.weekdays) ? body.weekdays.map(Number) : null;

    if (!instructorId) return res.status(400).json({ ok: false, error: "Falta instructorId" });
    // Si falta el rango, asumir desde hoy hasta 12 meses
    const todayStr = new Date().toISOString().slice(0, 10);
    if (!from) from = todayStr;
    if (!to) {
      const dt = new Date(todayStr + "T00:00:00");
      dt.setMonth(dt.getMonth() + 12);
      to = dt.toISOString().slice(0, 10);
    }
    if (!/^\d{4}-\d{2}-\d{2}$/.test(from) || !/^\d{4}-\d{2}-\d{2}$/.test(to)) {
      return res.status(400).json({ ok: false, error: "Rango de fechas inválido" });
    }
    if (!/^\d{2}:\d{2}$/.test(startTime) || !/^\d{2}:\d{2}$/.test(endTime)) {
      return res.status(400).json({ ok: false, error: "Formato de hora inválido (HH:MM)" });
    }
    if (startTime >= endTime) {
      return res.status(400).json({ ok: false, error: "La hora de fin debe ser posterior a la de inicio" });
    }

    // validar estilista del tenant
    const [[sty]] = await pool.query(
      "SELECT id FROM instructor WHERE id=? AND tenant_id=? LIMIT 1",
      [instructorId, tenantId]
    );
    if (!sty) return res.status(404).json({ ok:false, error:"Peluquero no encontrado en tu cuenta" });

    const startDate = new Date(`${from}T00:00:00`);
    const endDate = new Date(`${to}T00:00:00`);
    if (!(startDate instanceof Date) || isNaN(startDate) || !(endDate instanceof Date) || isNaN(endDate)) {
      return res.status(400).json({ ok: false, error: "Fechas inválidas" });
    }

    const conn = await pool.getConnection();
    try {
      await conn.beginTransaction();
      let created = 0;

      for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
        const wd = d.getDay(); // 0..6
        if (weekdays && weekdays.length && !weekdays.includes(wd)) {
          continue;
        }
        const dateStr = d.toISOString().slice(0, 10);
        const starts_at = `${dateStr}T${startTime}:00`;
        const ends_at = `${dateStr}T${endTime}:00`;

        // Evitar duplicados exactos
        const [[existing]] = await conn.query(
          `SELECT id FROM time_off 
           WHERE tenant_id=? AND instructor_id=? AND starts_at=? AND ends_at=? LIMIT 1`,
          [tenantId, instructorId, starts_at, ends_at]
        );
        if (existing) continue;

        await conn.query(
          `INSERT INTO time_off (tenant_id, instructor_id, starts_at, ends_at, reason)
           VALUES (?,?,?,?,?)`,
          [tenantId, instructorId, starts_at, ends_at, reason]
        );
        created += 1;
      }

      await conn.commit();
      return res.json({ ok: true, created });
    } catch (e) {
      await conn.rollback();
      console.error("[POST /api/days-off/recurring] error:", e);
      return res.status(500).json({ ok: false, error: "Error al crear bloqueos repetitivos" });
    } finally {
      conn.release();
    }
  } catch (e) {
    console.error("[POST /api/days-off/recurring] error:", e);
    return res.status(500).json({ ok: false, error: e.message });
  }
});

/**
 * POST /api/days-off
 * Body: { instructorId, starts_at, ends_at, reason }
 */
daysOff.post("/", async (req, res) => {
  try {
    const tenantId = req.tenant.id;
    const { instructorId, starts_at, ends_at, reason } = req.body;

    if (!instructorId) {
      return res.status(400).json({ ok: false, error: "Falta instructorId" });
    }
    if (!starts_at || !ends_at) {
      return res.status(400).json({ ok: false, error: "Faltan starts_at y/o ends_at" });
    }

    // validar estilista
    const [[instructor]] = await pool.query(
      `SELECT id FROM instructor WHERE id = ? AND tenant_id=?`,
      [instructorId, tenantId]
    );
    if (!instructor) {
      return res.status(404).json({ ok: false, error: "Peluquero no encontrado en tu cuenta" });
    }

    // normalizar fechas ISO → MySQL
    const normalize = (dt) =>
      typeof dt === "string" && dt.includes("T")
        ? dt.replace("T", " ").slice(0, 19)
        : dt;

    const sAt = normalize(starts_at);
    const eAt = normalize(ends_at);

    if (new Date(eAt) <= new Date(sAt)) {
      return res.status(400).json({ ok: false, error: "La hora/fecha de fin debe ser posterior a la de inicio" });
    }

    // solapes
    const [overlaps] = await pool.query(
      `SELECT id FROM time_off
       WHERE tenant_id=? AND instructor_id = ?
         AND (starts_at < ? AND ends_at > ?)`,
      [tenantId, instructorId, eAt, sAt]
    );
    if (overlaps.length) {
      return res.status(400).json({ ok: false, error: "Ya existe un bloqueo en ese horario" });
    }

    const [result] = await pool.query(
      `INSERT INTO time_off (tenant_id, instructor_id, starts_at, ends_at, reason, created_at)
       VALUES (?,?,?,?,?, NOW())`,
      [tenantId, instructorId, sAt, eAt, reason || "Bloqueo de tiempo"]
    );

    return res.json({ ok: true, id: result.insertId, message: "Bloqueo creado" });
  } catch (e) {
    console.error("[POST /api/days-off] error:", e);
    return res.status(500).json({ ok: false, error: e.message || "Error al crear bloqueo" });
  }
});

/**
 * PUT /api/days-off/:id
 */
daysOff.put("/:id", async (req, res) => {
  try {
    const tenantId = req.tenant.id;
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ ok:false, error:"ID inválido" });

    const { starts_at, ends_at, reason } = req.body;

    const [[block]] = await pool.query(
      `SELECT * FROM time_off WHERE id = ? AND tenant_id=?`,
      [id, tenantId]
    );
    if (!block) return res.status(404).json({ ok:false, error:"Bloqueo no encontrado" });

    const normalize = (dt) =>
      typeof dt === "string" && dt.includes("T")
        ? dt.replace("T", " ").slice(0, 19)
        : dt;

    const sAt = starts_at ? normalize(starts_at) : block.starts_at;
    const eAt = ends_at   ? normalize(ends_at)   : block.ends_at;

    if (new Date(eAt) <= new Date(sAt)) {
      return res.status(400).json({ ok:false, error:"La hora/fecha de fin debe ser posterior a la de inicio" });
    }

    await pool.query(
      `UPDATE time_off 
          SET starts_at=?, ends_at=?, reason=? 
        WHERE id=? AND tenant_id=?`,
      [sAt, eAt, reason ?? block.reason, id, tenantId]
    );

    return res.json({ ok:true, message:"Bloqueo actualizado" });
  } catch (e) {
    console.error("[PUT /api/days-off/:id] error:", e);
    return res.status(500).json({ ok:false, error:e.message || "Error al actualizar bloqueo" });
  }
});

/**
 * DELETE /api/days-off/:id
 */
daysOff.delete("/:id", async (req, res) => {
  try {
    const tenantId = req.tenant.id;
    const id = Number(req.params.id);
    if (!id) return res.status(400).json({ ok:false, error:"ID inválido" });

    const [r] = await pool.query(
      `DELETE FROM time_off WHERE id = ? AND tenant_id = ?`,
      [id, tenantId]
    );

    if (!r.affectedRows) {
      return res.status(404).json({ ok:false, error:"Bloqueo no encontrado" });
    }
    return res.json({ ok:true, message:"Bloqueo eliminado" });
  } catch (e) {
    console.error("[DELETE /api/days-off/:id] error:", e);
    return res.status(500).json({ ok:false, error:e.message || "Error al eliminar bloqueo" });
  }
});
